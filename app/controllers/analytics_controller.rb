class AnalyticsController < ApplicationController
  ...
  def index
    @completions_by_day = Rails.cache.fetch("analytics/lesson_completed_7_days", expires_in: 5.minutes) do
      AnalyticsEvent.where(event_type: "user.lesson_completed").where("created_at >= ?", 7.days.ago)
                    .group_by_day(:created_at).size
    end

    @top_paths = Rails.cache.fetch("analytics/top_paths", expires_in: 5.minutes) do
